  ...
  try {
    ...
    if (isNaN(userId)) {
      return NextResponse.json(
        {
          success: false,
          error: "Некоректний ID користувача (має бути числовим)",
        },
        { status: 400 }
      )
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: errorData.message || "Не вдалося оновити дані користувача",
        },
